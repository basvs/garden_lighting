...
async def _setup(hass, entry_data, light_state=STATE_OFF, attributes=None):
...
async def test_daylight_leaves_the_lamps_off(hass, entry_data, at_elevation):
    at_elevation(20.0)
...
async def test_dusk_drives_the_lamps(hass, entry_data, at_elevation):
    at_elevation(-4.0)
    turn_on = async_mock_service(hass, LIGHT_DOMAIN, SERVICE_TURN_ON)
    await _setup(hass, entry_data)
...
async def test_transition_is_sent_when_the_light_supports_it(hass, entry_data, at_elevation):
    at_elevation(-4.0)
    turn_on = async_mock_service(hass, LIGHT_DOMAIN, SERVICE_TURN_ON)
    await _setup(hass, entry_data, attributes={**DIMMABLE, "supported_features": 32})

    assert turn_on[0].data["transition"] == entry_data["update_interval"]


async def test_colour_temp_is_clamped_to_the_lamp(hass, entry_data, at_elevation):
    at_elevation(-6.0)  # fade essentially complete, so it wants the warm end
    turn_on = async_mock_service(hass, LIGHT_DOMAIN, SERVICE_TURN_ON)
    await _setup(
        hass,
        {**entry_data, "color_temp_enabled": True},
        attributes={
...
async def test_night_turns_a_lit_lamp_off_at_dawn(hass, entry_data, at_elevation):
    at_elevation(20.0)
...
async def test_entities_are_created(hass, entry_data, at_elevation):
    at_elevation(-4.0)
...
async def test_unload(hass, entry_data, at_elevation):
    at_elevation(-4.0)
    async_mock_service(hass, LIGHT_DOMAIN, SERVICE_TURN_ON)
...
class TestManualControl:
    ...
    async def _dusk(self, hass, entry_data, at_elevation):
        at_elevation(-4.0)
        turn_on = async_mock_service(hass, LIGHT_DOMAIN, SERVICE_TURN_ON)
        entry = await _setup(hass, entry_data)
    ...
    async def test_a_hand_change_takes_the_light(self, hass, entry_data, at_elevation):
        coordinator, turn_on, commanded = await self._dusk(hass, entry_data, at_elevation)
    ...
    async def test_switching_it_off_by_hand_keeps_it_off(self, hass, entry_data, at_elevation):
        coordinator, turn_on, _ = await self._dusk(hass, entry_data, at_elevation)
    ...
    async def test_our_own_changes_are_not_mistaken_for_a_person(
        ...
        coordinator, _, commanded = await self._dusk(hass, entry_data, at_elevation)
    ...
    async def test_a_small_drift_is_not_a_person(self, hass, entry_data, at_elevation):
        coordinator, _, commanded = await self._dusk(hass, entry_data, at_elevation)
    ...
    async def test_the_reset_service_gives_control_back(self, hass, entry_data, at_elevation):
        coordinator, turn_on, _ = await self._dusk(hass, entry_data, at_elevation)
    ...
    async def test_daylight_clears_it_for_the_next_evening(
        self, hass, entry_data, at_elevation, monkeypatch
    ):
        coordinator, _, _ = await self._dusk(hass, entry_data, at_elevation)
